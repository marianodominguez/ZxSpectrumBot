...
def run_emulator(api, tweet, config):
    ...
    if os.path.exists("working/movie.fmf"):
        ...
        result = os.system(f'ffmpeg -y -i working/OUTPUT_BIG.mp4 -c:v libx264 -pix_fmt yuv420p -r 30 -vf "scale=w=640:pad=ceil(iw/2)*2:ceil(ih/2)*2" -sws_flags accurate_rnd -b:v 5M -t 2:20 -acodec aac -ar 44100 -ac 2 working/OUTPUT_SMALL.mp4')
        logger.debug(result)
    else:
