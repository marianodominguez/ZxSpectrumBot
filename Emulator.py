...
def run_emulator(api, tweet, config):
    ...
    if movie_support:
        cmd = f'/usr/bin/fuse --fbmode 640 --graphics-filter 2x --sound-device alsa:card=0 --speed {speed*100} --no-confirm-actions --no-autosave-settings --auto-load --movie-start working/movie.fmf --rate 2 --sound-freq 44100 --separation ACB --tape working/tape.tap'.split()
