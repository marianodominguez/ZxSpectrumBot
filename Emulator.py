...
def run_emulator(api, tweet, config):
    ...
    if os.path.exists("working/movie.fmf"):
        ...
        result = os.system(f'ffmpeg -y -i working/OUTPUT_BIG.mp4 -pix_fmt yuv420p -vcodec h264 -crf 2 -preset slow -profile:v high -b:v 5000k -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" -strict experimental -r 30 -t 2:20 -acodec aac -ar 44100 -ac 2 working/OUTPUT_SMALL.mp4')
        logger.debug(result)
    else:
